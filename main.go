...
func main() {
	...
	contents := formatWithName(name, typeMeta, aliasedTypes)
...
func formatWithName(name string, typeMeta Node, aliases map[string]*NodeAlias) string {
...
func fieldNameForName(name string) string {
	...
	for x := 0; x < len(nameRunes); x++ {
		...
		if unicode.IsSpace(nameRunes[x]) {
			continue
		}
		if nextUppercase {
			outputRunes = append(outputRunes, unicode.ToUpper(nameRunes[x]))
